...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  function openBrand(data,module,brand,meta,source='cloud'){
    const regionRows=filterRegion(data),viewRows=S.compareYears?compareRows(regionRows,S.compareTarget,S.compareTargetMonth):filterPeriod(regionRows),rows=viewRows.filter(r=>managerName(r)===brand),t=totals(rows,module),accounts=groupRows(rows,accountName),labels=periodMetricLabels();
