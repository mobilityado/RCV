...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  function renderData(data,module,meta={},source='cloud'){
    ...
    const t=totals(rows,module),accounts=groupRows(rows,accountName),bad=[...accounts].filter(([,rs])=>totals(rs,module).st==='red').length,good=accounts.size-bad,compliance=pctLabel(rows,module);
    ...
    const head=`<div class="rcv34-breadcrumb"><span>Región</span><b>${esc(adminRegionLabel())}</b><i>›</i><strong>Cuentas contables</strong></div>
      <div class="rcv42-kpis"><article><small>REAL</small><strong>${money(t.real)}</strong><span>${esc(activeLabel)}</span></article><article><small>PRESUPUESTO</small><strong>${money(t.budget)}</strong><span>Periodo seleccionado</span></article><article><small>VARIACIÓN</small><strong>${money(t.diff)}</strong></article><article><small>${module==='productividad'?'CUMPLIMIENTO':'USO DE PRESUPUESTO'}</small><strong>${compliance}</strong><span>${bad} cuentas rojas · ${good} verdes</span></article><article class="${bad?'alert':''}"><small>CUENTAS ROJAS</small><strong>${bad}</strong><span>de ${accounts.size} cuentas</span></article></div>`;
    ...
    const accountItems=[...accounts].sort((a,b)=>Math.abs(totals(b[1],module).real)-Math.abs(totals(a[1],module).real)).map(([name,rs])=>{
      const z=totals(rs,module),affected=new Set(rs.map(managerName).filter(Boolean)).size,concepts=new Set(rs.map(accountDetailName).filter(Boolean)).size;
      return `<article class="rcv34-break-item r50-account-card ${z.st==='red'?'is-red':'is-green'}" data-r50-account="${esc(name)}" data-unit-state="${z.st}"><div class="rcv34-break-top"><div><strong>${esc(name)}</strong><small>${affected} gerencia(s) afectadas · ${concepts} concepto(s) · ${rs.length.toLocaleString()} movimientos</small>${redStreak(rs,module)>=2?`<em class="rcv43-reincide">↻ ${redStreak(rs,module)} meses en rojo</em>`:''}</div>${sema(z.st)}</div><div class="amounts"><span>Real<b>${money(z.real)}</b></span><span>Presupuesto<b>${money(z.budget)}</b></span><span>Diferencia<b>${money(z.diff)}</b></span></div></article>`
    }).join('');
    ...
    host.insertAdjacentHTML('beforeend',r503ContextChips(module,source)+periodControls(regionRows)+compareExecutiveBlock(regionRows,module)+executiveReportBar(rows,module,meta,source)+head+
      ...
      chart+
      `<article class="rcv34-card"><div class="rcv34-card-head"><div><span class="r503-section-kicker">ANÁLISIS CONTABLE</span><h3>Cuentas contables de la región</h3><p>Selecciona una cuenta para ver exactamente a qué gerencias, áreas, centros y conceptos afecta; cada afectación conserva su semáforo y seguimiento.</p></div>${noteBtn}</div>
        <div class="r473-unit-filter"><div><span>FILTRAR CUENTAS</span><strong id="r473UnitCount">${accounts.size} de ${accounts.size} visibles</strong></div><div class="r473-filter-buttons"><button class="active" data-r473-filter="all">Todas <b>${accounts.size}</b></button><button class="red" data-r473-filter="red">Rojas <b>${bad}</b></button><button class="green" data-r473-filter="green">Verdes <b>${good}</b></button></div></div>
        <div class="rcv34-breakdown r50-account-grid" id="r473UnitGrid">${accountItems}</div><div id="r473UnitEmpty" class="r473-empty rcv34-hidden">No hay cuentas con el estado seleccionado.</div>
      </article>`);
    bindPeriodControls();bindUnitStateFilter(accounts.size);bindExecutiveReport(rows,module,meta,source);drawCharts(accounts,module);drawTrendChart($('r42TrendChart'),rows,module);bindTopActions(data,module,meta,source);
