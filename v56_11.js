...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  function compareExecutiveBlock(regionRows,module){
    ...
    return `<section class="rcv481-compare r50-compare r562-compare">
      ...
      <article class="rcv34-card r562-compare-card">
        <div class="rcv42-top-head"><div><h3>Desglose comparativo por ${mode==='brand'?'marca afectada':'cuenta contable'}</h3><p>Elige el orden de navegación. Después haz clic en una fila para continuar al siguiente nivel.</p></div><span>${items.length} ${countLabel}</span></div>
  ...
  function renderData(data,module,meta={},source='cloud'){
    ...
    bindPeriodControls();bindBreakdownMode();bindUnitStateFilter(activeGroups.size);bindExecutiveReport(rows,module,meta,source);drawCharts(accounts,module);drawTrendChart($('r42TrendChart'),rows,module);bindTopActions(data,module,meta,source);
  ...
  async function publishLocal(module){
    ...
    try{
      ...
      // v56.11: Base64 URL-safe evita que + / = se alteren durante POST -> Apps Script.
