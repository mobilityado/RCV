(() => {
  ...
  async function post(params){
    // v55.5: transporte POST restaurado desde la versión que publicaba correctamente: POST real por formulario oculto + verificación prolongada del snapshot. En algunos navegadores,
...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  async function publishLocal(module){
    ...
    try{
      ...
      // v55.5: envío por bloques pequeños. Evita que Apps Script pierda/trunque un POST grande.
      const uploadId='U'+Date.now()+'_'+Math.random().toString(36).slice(2);
      const PART=28000, parts=[]; for(let i=0;i<packed.length;i+=PART)parts.push(packed.slice(i,i+PART));
      ...
      msg.textContent='Armando publicación en Google Sheets…';
      await post({accion:'v55_upload_finish',token:S.session.token,module,uploadId,total:parts.length});
      let confirmed=null;
      for(let i=0;i<60;i++){
        msg.textContent='Confirmando publicación… '+(i+1)+'/60'; await new Promise(r=>setTimeout(r,1500));
        try{const h=await jsonpLong({accion:'v34_history',token:S.session.token,module},30000);const latest=h?.items?.[0];if(latest?.snapshotId&&latest.snapshotId!==beforeId){confirmed=latest;break}}catch(_){}
      }
      if(!confirmed)throw new Error('La carga llegó por bloques, pero la nube no confirmó el snapshot. Revisa Ejecuciones de Apps Script.');
      msg.textContent='✓ Publicación confirmada · '+String(confirmed.snapshotId).slice(0,8)+' · Regiones: '+regs.join(', ');msg.className='rcv34-msg ok';S.adminSnapshot='';await loadHistory(module);
