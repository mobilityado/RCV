...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  async function renderMenu(){
    ...
    const hero=`<section class="r49-hero"><div class="r49-hero-copy"><span class="r49-eyebrow">CENTRO DE INTELIGENCIA REGIONAL</span><h2>Control ejecutivo RCV</h2><p>Consulta Gastos, Costos y Productividad con navegación por Región → Gerencia → Área, comparativa interanual y seguimiento de semáforos.</p></div><div class="r49-hero-badge"><small>VERSIÓN</small><strong>49.0</strong><span>Diseño ejecutivo</span></div></section>`;
  ...
  function bindPeriodControls(){
    ...
    if(y)y.onchange=rerender;if(f)f.onchange=rerender;if(t)t.onchange=rerender;if(base)base.onchange=rerender;if(target)target.onchange=rerender;const ft=document.querySelector('.r49-filter-toggle'),fc=document.querySelector('.r49-filter-card');if(ft&&fc)ft.onclick=()=>fc.classList.toggle('open');
  ...
  async function loadGeneral(){
    ...
    try{
      ...
      for(const module of mods){
        ...
        const redManagers=[...groupRows(rows,managerName)].filter(([,rs])=>totals(rs,module).st==='red').length;

        moduleCards+=`<article class="rcv34-menu-card" data-general-open="${module}">
          <div class="ico">${MODULES[module].icon}</div>
          <h3>${MODULES[module].label}</h3>
          <p>Real <b>${money(t.real)}</b><br>Presupuesto <b>${money(t.budget)}</b><br><b>${redManagers}</b> gerencias rojas</p>
      ...
      if(admin && allRegions.size){
        ...
        regional=`<article class="rcv34-card rcv42-regional">
          <div class="rcv42-top-head">
            <div><h3>Comparativo entre regiones</h3><p>Selecciona una región para revisar su estructura Región → Gerencia → Área → Detalle.</p></div>
