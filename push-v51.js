(() => {
  ...
  async function registerSW(){if(!('serviceWorker'in navigator))throw new Error('Este navegador no admite Service Worker.');if(!swReg)swReg=await navigator.serviceWorker.register('./firebase-messaging-sw.js',{scope:'./'});return swReg}
  ...
  async function syncCurrentToken(pt,force=false){
    ...
    const key='reportia_push_sync_v532',now=Date.now(),last=Number(localStorage.getItem(key)||0);
  ...
  async function testPush(){
    ...
    try{
      ...
      if(msg){const dep=Number(d.depurados||0),fall=Number(d.fallidos||0),env=Number(d.enviados||0),dest=Number(d.destinatarios||0);let t=`Servidor ${d.serverVersion||'v53.2'} · encontrados ${dest} · aceptados ${env} · vencidos ${dep} · otros errores ${fall}.`;if(dep)t+=` Los tokens vencidos fueron desactivados automáticamente.`;if(dest===0)t+=` Abre REPORT.IA en cada dispositivo para registrarlo.`;msg.textContent=t}
      return d;
