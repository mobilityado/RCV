...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  function compareExecutiveBlock(regionRows,module){
    ...
    const aRows=compareRows(regionRows,S.compareBase,S.compareBaseMonth),bRows=compareRows(regionRows,S.compareTarget,S.compareTargetMonth),c=compareTotals(regionRows,module);
    const names=[...new Set([...aRows.map(accountName),...bRows.map(accountName)])];
    const accounts=names.map(name=>{
      const a=totals(aRows.filter(r=>accountName(r)===name),module),b=totals(bRows.filter(r=>accountName(r)===name),module),metric=compareMetric(module,a.real,b.real),trend=evolutionTag(module,a,b);
      return{name,a,b,delta:metric.delta,pct:metric.pct,trend}
    }).sort((x,y)=>Math.abs(y.delta)-Math.abs(x.delta));
    const improved=accounts.filter(x=>x.trend.kind==='good').length,worse=accounts.filter(x=>x.trend.kind==='bad').length,unchanged=accounts.length-improved-worse;
    ...
    return `<section class="rcv481-compare r50-compare">
  ...
  function renderData(data,module,meta={},source='cloud'){
    ...
    host.querySelectorAll('[data-r50-compare-account]').forEach(el=>el.onclick=()=>openAccount(data,module,el.dataset.r50CompareAccount,meta,source));
  ...
  async function publishLocal(module){
    ...
    try{
      ...
      // v55.9: la finalización se hace por GET/JSONP, que ya comprobamos que sí llega a Apps Script.
