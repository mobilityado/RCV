(() => {
  ...
  const S={session:null,current:'menu',module:null,local:null,cloud:null,cloudMeta:null,history:[],adminRegion:'',adminSnapshot:'',adminSource:'cloud',notificationTimer:null,periodYear:'',periodFrom:'',periodTo:''};
  ...
  function periodControls(rows){
    ...
    const months=[...new Set((rows||[]).map(r=>monthOrder(r.period)).filter(m=>m>=1&&m<=12))].sort((a,b)=>a-b);
    if(!years.length&&!months.length)return'';
    const selected=String(S.periodYear||latestYear(rows)||'');
    const yearOpts='<option value="">Todos</option>'+years.map(y=>`<option value="${esc(y)}" ${selected===y?'selected':''}>${esc(y)}</option>`).join('');
    ...
    return `<article class="rcv34-card rcv43-period-card"><div class="rcv43-period-title"><div><b>Filtro del periodo del archivo</b><span>El archivo contiene ${years.length?years.join(' y '):'el periodo disponible'}. KPIs, gráficas, semáforos y detalle se recalculan con el año seleccionado.</span></div><button id="r43PeriodClear" class="rcv34-btn">Ver año más reciente</button></div><div class="rcv43-period-grid"><label>Año<select id="r43Year">${yearOpts}</select></label><label>Desde<select id="r43From">${monthOpts(S.periodFrom,'Inicio')}</select></label><label>Hasta<select id="r43To">${monthOpts(S.periodTo,'Fin')}</select></label><div class="rcv43-period-hint"><b>${periodRangeLabel()}</b><small>${S.periodFrom&&S.periodTo&&String(S.periodFrom)===String(S.periodTo)?'Comparación contra mes anterior activada':'Puedes consultar 2025, 2026 o un rango de meses'}</small></div></div></article>`;
  }
  ...
  function bindPeriodControls(){
    const y=$('r43Year'),f=$('r43From'),t=$('r43To'),c=$('r43PeriodClear');
    const rerender=()=>{S.periodYear=y?.value||'';S.periodFrom=f?.value||'';S.periodTo=t?.value||'';if(S.periodFrom&&S.periodTo&&Number(S.periodFrom)>Number(S.periodTo)){const z=S.periodFrom;S.periodFrom=S.periodTo;S.periodTo=z}renderCurrentSource()};
    if(y)y.onchange=rerender;if(f)f.onchange=rerender;if(t)t.onchange=rerender;if(c)c.onclick=()=>{S.periodYear='';S.periodFrom='';S.periodTo='';renderCurrentSource()};
  }
  ...
  function renderData(data,module,meta={},source='cloud'){
    ...
    const regionRows=filterRegion(data);if(!S.periodYear){const ly=latestYear(regionRows);if(ly)S.periodYear=ly}
  ...
  async function exportExecutiveExcel(rows,module,meta,source){
    ...
    const dashboard=[
      ...
      ['Jerarquías rojas',red.length],['Jerarquías verdes',green],['Reincidentes',repeat.length],
