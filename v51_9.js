    ...
    try{
      ...
      $('r34Panel').innerHTML=`
        <section class="r519-device-card">
          ...
          <div class="r519-device-actions"><button id="r519EnableDevicePush" class="rcv34-btn primary">🔔 Activar en este dispositivo</button><button id="r519LocalTest" class="rcv34-btn">🧪 Probar aviso local</button><button id="r519OpenProfile" class="rcv34-btn">👤 Mi perfil</button></div>
          <p id="r519DeviceMsg" class="r519-device-msg">La prueba local sirve para separar un bloqueo de Windows/Android de un problema de Firebase.</p>
        </section>
      ...
      $('r519LocalTest').onclick=async()=>{const m=$('r519DeviceMsg');m.textContent='Enviando prueba local…';try{await window.REPORTIA_PUSH?.testLocal?.();m.textContent='Prueba local enviada. Si no aparece, revisa los permisos del sistema operativo o del navegador.'}catch(e){m.textContent=e.message||String(e)}};
