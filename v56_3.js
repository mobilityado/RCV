...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  // v56.3: la Marca afectada debe usar el nombre descriptivo de la Unidad de Negocio, no el código numérico de la jerarquía de sublibro.
  function brandName(r){const name=norm(r.brand||r.businessUnit||'');if(name)return name.replace(/^\d+(?:\.\d+)*[.\-\s]+/,'').trim()||name;const code=norm(r.businessUnitCode||'');if(code)return code;return managerName(r)}
    ...
    return `<section class="rcv481-compare r50-compare r562-compare">
      ...
      <article class="rcv34-card r562-compare-card">
        ...
        <div class="r50-compare-table r562-compare-table"><div class="head"><span>${firstLabel}</span><span>${esc((S.compareBaseMonth?MONTH_LABELS[Number(S.compareBaseMonth)]+' ':'')+S.compareBase)}</span><span>${esc((S.compareTargetMonth?MONTH_LABELS[Number(S.compareTargetMonth)]+' ':'')+S.compareTarget)}</span><span>Diferencia (+/-)</span><span>Variación (+/-)</span><span>Resultado</span></div>
          ${items.map(x=>{const semKind=x.trend.kind==='bad'?'bad':'good',attr=mode==='brand'?`data-r562-compare-brand="${esc(x.name)}"`:`data-r50-compare-account="${esc(x.name)}"`;return `<button class="row" ${attr}><b>${esc(x.name)}</b><span>${money(x.a.real)}</span><span>${money(x.b.real)}</span><strong class="${x.trend.kind}">${x.delta>=0?'+':''}${money(x.delta)}</strong><span class="${semKind}">${x.pct>=0?'+':''}${x.pct.toFixed(1)}%</span><em class="r534-result-dot ${semKind}" title="${esc(x.trend.label)}" aria-label="${esc(x.trend.label)}"></em></button>`}).join('')}
        </div>
      </article>
    </section>`;
  }
  ...
  function openBrand(data,module,brand,meta,source='cloud'){
    const regionRows=filterRegion(data),viewRows=S.compareYears?compareRows(regionRows,S.compareTarget,S.compareTargetMonth):filterPeriod(regionRows),rows=viewRows.filter(r=>brandName(r)===brand),t=totals(rows,module),accounts=groupRows(rows,accountName);
    $('r34DetailTitle').textContent=brand;$('r34DetailSub').textContent=`${MODULES[module].label} · ${adminRegionLabel()} · Marca afectada`;
  ...
  function renderData(data,module,meta={},source='cloud'){
    ...
    const t=totals(rows,module),accounts=groupRows(rows,accountName),brands=groupRows(rows,brandName),mode=breakdownMode(),activeGroups=mode==='brand'?brands:accounts,bad=[...activeGroups].filter(([,rs])=>totals(rs,module).st==='red').length,good=activeGroups.size-bad,compliance=pctLabel(rows,module);
  ...
  async function publishLocal(module){
    ...
    try{
      ...
      // v56.3: la finalización se hace por GET/JSONP, que ya comprobamos que sí llega a Apps Script.
