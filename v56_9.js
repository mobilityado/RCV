...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  async function publishLocal(module){
    ...
    try{
      ...
      const raw=JSON.stringify(S.local), packed=await gzipBase64(raw);
      ...
      const PART=45000, parts=[]; for(let i=0;i<packed.length;i+=PART)parts.push(packed.slice(i,i+PART));
