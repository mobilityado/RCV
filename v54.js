(() => {
  ...
  function repairRegionData(data){
    if(!data)return data;
    const copy={...data,rows:(data.rows||[]).map(repairRegionRow)};
...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  async function publishLocal(module){
    ...
    try{
      ...
      if(!confirmed)throw new Error('La nube NO confirmó una publicación nueva. El portal evitó mostrar “publicado correctamente”. Revisa que el Apps Script v54.5 esté implementado como nueva versión y que config.js apunte a esa implementación.');
