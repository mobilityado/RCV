...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  async function publishLocal(module){if(!S.local)return;const b=$('r34Publish'),msg=$('r34UploadMsg');b.disabled=true;msg.textContent='Publicando…';try{S.local=repairRegionData(S.local);await post({accion:'v34_upload',token:S.session.token,module,payload:JSON.stringify(S.local)});msg.textContent='Publicación enviada correctamente a la nube.';msg.className='rcv34-msg ok';setTimeout(()=>{loadHistory(module);b.disabled=false},1200)}catch(e){msg.textContent=e.message;msg.className='rcv34-msg error';b.disabled=false}}
