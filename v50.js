...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  function openAccount(data,module,account,meta,source='cloud'){
    ...
    $('r34DetailTitle').textContent=account;
    $('r34DetailSub').textContent=`${MODULES[module].label} · ${adminRegionLabel()} · Cuenta contable → impactos`;
    const affectedCards=[...affected].sort((a,b)=>Math.abs(totals(b[1],module).real)-Math.abs(totals(a[1],module).real)).map(([name,rs])=>{
      ...
      return `<article class="r50-impact-card ${z.st==='red'?'is-red':'is-green'}"><div class="top"><div><strong>${esc(name)}</strong><small>${areaList.map(esc).join(' · ')||'Sin área identificada'}</small></div>${sema(z.st)}</div><div class="amounts"><span>Real<b>${money(z.real)}</b></span><span>Presupuesto<b>${money(z.budget)}</b></span><span>Diferencia<b>${money(z.diff)}</b></span></div></article>`
    }).join('');
    const conceptCards=[...concepts].sort((a,b)=>Math.abs(totals(b[1],module).real)-Math.abs(totals(a[1],module).real)).map(([name,rs])=>{
      const z=totals(rs,module);return `<article class="rcv34-subhier ${z.st==='red'?'is-red':'is-green'}"><div><strong>${esc(name)}</strong>${sema(z.st)}</div><span>Real <b>${money(z.real)}</b></span><span>Presupuesto <b>${money(z.budget)}</b></span></article>`
    }).join('');
    ...
    $('r34DetailBody').innerHTML=`<div class="rcv48-breadcrumb"><b>${esc(adminRegionLabel())}</b><i>›</i><b>Cuenta contable</b><i>›</i><span>${esc(account)}</span></div>
      ...
      <article class="rcv34-card"><div class="rcv42-top-head"><div><h3>¿A qué afecta esta cuenta?</h3><p>Gerencias y áreas que registran movimientos de la cuenta, cada una con su semáforo.</p></div><span>${affected.size} gerencias</span></div><div class="r50-impact-grid">${affectedCards||'<div class="rcv34-empty">Sin impactos identificados.</div>'}</div></article>
      <article class="rcv34-card"><h3>Conceptos que integran la cuenta</h3><p>Desglose de cuentas / conceptos detallados dentro de ${esc(account)}.</p><div class="rcv34-subhier-grid">${conceptCards}</div></article>
      <article class="rcv34-card"><div class="rcv41-detail-head"><div><h3>Movimientos de la cuenta</h3><p id="r41DetailCount">${rows.length.toLocaleString()} movimientos mostrados de ${rows.length.toLocaleString()}.</p></div><div class="rcv42-detail-tools"><div class="rcv41-detail-filters"><button class="rcv41-state-btn active" data-r41-state="all">Todos <b>${rows.length}</b></button><button class="rcv41-state-btn red" data-r41-state="red">Rojos <b>${redRows}</b></button><button class="rcv41-state-btn green" data-r41-state="green">Verdes <b>${greenRows}</b></button></div><input id="r42DetailSearch" class="rcv42-search" placeholder="Buscar concepto, gerencia, área, centro…"></div></div>
        <div class="rcv34-table-wrap"><table class="rcv34-table"><thead><tr><th>Cuenta / concepto</th><th>Gerencia</th><th>Área</th><th>Centro / afectación</th><th>Mes</th><th>Real</th><th>Presupuesto</th><th>Diferencia</th><th>Estado</th></tr></thead><tbody id="r41DetailRows">${makeRows(rows)}</tbody></table></div><div id="r41DetailEmpty" class="rcv41-detail-empty rcv34-hidden">No hay movimientos con el filtro seleccionado.</div>
      </article>${plan}${conversation}`;
    $('r34Detail').classList.add('open');
    let detailState='all';const applyDetailFilter=state=>{detailState=state;const q=upper($('r42DetailSearch')?.value||'');const trs=[...document.querySelectorAll('#r41DetailRows tr')];let shown=0;trs.forEach(tr=>{const okState=state==='all'||tr.dataset.detailState===state,textOk=!q||upper(tr.textContent).includes(q),show=okState&&textOk;tr.classList.toggle('rcv34-hidden',!show);if(show)shown++});document.querySelectorAll('[data-r41-state]').forEach(btn=>btn.classList.toggle('active',btn.dataset.r41State===state));if($('r41DetailCount'))$('r41DetailCount').textContent=`${shown.toLocaleString()} movimientos mostrados de ${rows.length.toLocaleString()}.`;if($('r41DetailEmpty'))$('r41DetailEmpty').classList.toggle('rcv34-hidden',shown>0)};
    ...
    if(conversation){loadThread(module,meta.snapshotId,adminRegionLabel(),key);$('r34SendNote').onclick=()=>sendNote(module,meta.snapshotId,adminRegionLabel(),key)}
  }
  function renderData(data,module,meta={},source='cloud'){
    ...
    const chart=`<div class="rcv42-chart-grid"><article class="rcv34-card rcv34-chart-card"><h3>Real vs presupuesto por cuenta</h3><p>Principales cuentas contables de ${esc(adminRegionLabel())}.</p><canvas id="r34CompareChart"></canvas></article><article class="rcv34-card rcv34-chart-card"><h3>Evolución mensual</h3><p>Comportamiento del Real contra Presupuesto.</p><canvas id="r42TrendChart"></canvas></article><article class="rcv34-card rcv34-chart-card"><h3>Estado de cuentas</h3><p>Distribución rojo / verde.</p><canvas id="r34StatusChart"></canvas><div class="rcv34-chart-legend"><span><i class="green"></i>${good} verdes</span><span><i class="red"></i>${bad} rojas</span></div></article>${topDeviationBlock(accounts,module)}</div>`;
    ...
    host.insertAdjacentHTML('beforeend',periodControls(regionRows)+compareExecutiveBlock(regionRows,module)+executiveReportBar(rows,module,meta,source)+head+
      `<div class="rcv48-ranking-grid">${rankingBlock(accounts,module,'Top 10 cuentas contables','Cuentas con mayor importe Real.')}${topDeviationBlock(accounts,module)}</div>`+
      chart+
    ...
    bindPeriodControls();bindUnitStateFilter(accounts.size);bindExecutiveReport(rows,module,meta,source);drawCharts(accounts,module);drawTrendChart($('r42TrendChart'),rows,module);
