...
(function(){
  ...
  async function exportPdf(element, filename, title){
    ...
    try{
      ...
      cover.innerHTML='<div style="padding:28px 30px;margin-bottom:20px;border-radius:20px;background:linear-gradient(135deg,#0f172a,#1e293b,#0f4c5c);color:white"><div style="font:800 11px system-ui;letter-spacing:.14em;color:#7dd3fc">REPORT.IA RCV</div><div style="font:850 30px system-ui;margin-top:8px">'+title+'</div><div style="font:500 12px system-ui;margin-top:7px;color:#cbd5e1">REPORT.IA RCV 18.0 · Clean Rebuild · Reporte analítico completo</div><div style="font:500 10px system-ui;margin-top:16px;color:#94a3b8">'+new Date().toLocaleString('es-MX')+'</div></div>';
      ...
      while(sy<canvas.height){
        ...
        pdf.text('REPORT.IA RCV · REPORT.IA RCV 18.0 · Clean Rebuild',margin,10);pdf.text('Página '+page,pw-margin,10,{align:'right'});
  ...
  async function generateFinalPackage(){
    ...
    try{
      ...
      if(typeof JSZip!=='undefined'){
        ...
        const summary=[
          'REPORT.IA RCV · REPORT.IA RCV 18.0 · Clean Rebuild',
          '',
          `Periodo: ${month}`,
...
// ===== REPORT.IA RCV 17.1 · CLEAN MODULE NAVIGATION =====
(function(){
  function showHome(){
    document.body.classList.add('ecc17-home');
    document.body.classList.remove('ecc17-module-view');
    document.querySelectorAll('.module-active').forEach(x=>x.classList.remove('module-active'));
    document.querySelectorAll('#ecc17SidebarNav button').forEach(x=>x.classList.toggle('active',x.dataset.target==='ecc17'));
    document.getElementById('ecc17')?.scrollIntoView({behavior:'smooth',block:'start'});
  }

  function showModule(id){
    if(id==='ecc17'){showHome();return;}
    document.body.classList.remove('ecc17-home');
    document.body.classList.add('ecc17-module-view');
    document.querySelectorAll('.module-active').forEach(x=>x.classList.remove('module-active'));
    const target=document.getElementById(id);
    if(target){
      target.classList.add('module-active');
      target.style.display='block';
      setTimeout(()=>target.scrollIntoView({behavior:'smooth',block:'start'}),50);
    }
    document.querySelectorAll('#ecc17SidebarNav button').forEach(x=>x.classList.toggle('active',x.dataset.target===id));
  }

  document.addEventListener('click',e=>{
    const nav=e.target.closest('#ecc17SidebarNav button');
    if(nav){
      e.preventDefault();
      showModule(nav.dataset.target);
      return;
    }

    const quick=e.target.closest('[data-report]');
    if(quick){
      showModule('report16');
      setTimeout(()=>document.querySelector('[data-r16="'+quick.dataset.report+'"]')?.click(),180);
    }

    const scroll=e.target.closest('[data-scroll]');
    if(scroll){
      showModule(scroll.dataset.scroll);
    }
  });
...
// ===== REPORT.IA RCV 18.0 · CLEAN SHELL GUARD =====
(function(){
  function cleanGeometry(){
    const body=document.body;
    const shell=document.querySelector('.v18-shell');
    const sidebar=shell?.querySelector(':scope > .sidebar');
    const main=shell?.querySelector(':scope > main');
    if(!shell||!sidebar||!main)return;

    // Cancel inline geometry inherited from older versions.
    [
      'display','grid-template-columns','gridTemplateColumns'
    ].forEach(p=>body.style.removeProperty(p));

    ['position','left','right','bottom','width','min-width','max-width','height','margin-left']
      .forEach(p=>sidebar.style.removeProperty(p));

    ['margin','margin-left','width','min-width','max-width','padding-left','padding-right']
      .forEach(p=>main.style.removeProperty(p));
  }
  ...
  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded',()=>setTimeout(initV18,0));
  }else{
    setTimeout(initV18,0);
  }
  window.addEventListener('resize',cleanGeometry);
})();
