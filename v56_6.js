...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  function compareExecutiveBlock(regionRows,module){
    ...
    const firstLabel=mode==='brand'?'Marca afectada':'Cuenta contable', countLabel=mode==='brand'?'gerencias':'cuentas';
    return `<section class="rcv481-compare r50-compare r562-compare">
      ...
      <article class="rcv34-card r562-compare-card">
        ...
        <div class="r50-compare-table r562-compare-table"><div class="head"><span>${firstLabel}</span><span>${esc((S.compareBaseMonth?MONTH_LABELS[Number(S.compareBaseMonth)]+' ':'')+S.compareBase)}</span><span>${esc((S.compareTargetMonth?MONTH_LABELS[Number(S.compareTargetMonth)]+' ':'')+S.compareTarget)}</span><span>Diferencia (+/-)</span><span>Variación (+/-)</span><span>Resultado</span></div>
          ${items.map(x=>{const semKind=x.trend.kind==='bad'?'bad':'good',attr=mode==='brand'?`data-r562-compare-brand="${esc(x.name)}"`:`data-r50-compare-account="${esc(x.name)}"`,label=mode==='brand'?brandDisplayName(x.name,[...aRows,...bRows].filter(r=>keyFn(r)===x.name)):x.name;return `<button class="row" ${attr}><b>${esc(label)}</b><span>${money(x.a.real)}</span><span>${money(x.b.real)}</span><strong class="${x.trend.kind}">${x.delta>=0?'+':''}${money(x.delta)}</strong><span class="${semKind}">${x.pct>=0?'+':''}${x.pct.toFixed(1)}%</span><em class="r534-result-dot ${semKind}" title="${esc(x.trend.label)}" aria-label="${esc(x.trend.label)}"></em></button>`}).join('')}
        </div>
      </article>
    </section>`;
  }
  ...
  function openBrand(data,module,brand,meta,source='cloud'){
    ...
    const brandLabel=brandDisplayName(brand,rows);$('r34DetailTitle').textContent=brandLabel;$('r34DetailSub').textContent=`${MODULES[module].label} · ${adminRegionLabel()} · Gerencia → Cuenta contable → Conceptos`;
  ...
  async function publishLocal(module){
    ...
    try{
      ...
      const h=await jsonpLong({accion:'v34_history',token:S.session.token,module},60000);
  ...
  async function loadGeneral(){
    ...
    try{
      ...
      const results=await Promise.allSettled(mods.map(module=>jsonpLong({accion:'v34_module',token:S.session.token,module},60000)));
      const d={modules:{}};
