...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  function topDeviationBlock(groups,module){
    const top=[...groups].map(([name,rs])=>({name,rs,dev:deviationValue(rs,module),t:totals(rs,module)})).filter(x=>x.dev>0).sort((a,b)=>b.dev-a.dev).slice(0,10);
