(() => {
  ...
  const S={session:null,current:'menu',module:null,local:null,cloud:null,cloudMeta:null,history:[],adminRegion:'',adminSnapshot:'',adminSource:'cloud',notificationTimer:null,periodYear:'',periodFrom:'',periodTo:'',compareYears:false,compareBase:'',compareTarget:'',compareBaseMonth:'',compareTargetMonth:''};
...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  async function renderNotifications(){
    ...
    try{
      ...
      const html=items.length?items.map(x=>`<article class="rcv34-notif-item ${x.unread?'unread':''}" data-notif-module="${esc(x.module)}"><div class="rcv34-notif-icon">${upper(x.tipo)==='ADMINISTRADOR'?'A':'R'}</div><div class="rcv34-notif-main"><div><strong>${esc(x.hierarchy)}</strong><span>${esc(x.fecha)}</span></div><p>${esc(x.texto)}</p><small>${esc(x.usuario)} · ${esc(x.gerenciaDestino||x.region)} · ${esc(MODULES[x.module]?.label||x.module)}</small></div><span class="rcv42-status ${upper(x.estado)==='ATENDIDA'?'done':upper(x.estado)==='EN SEGUIMIENTO'?'tracking':'pending'}">${esc(x.estado||'PENDIENTE')}</span>${x.unread?'<b class="rcv34-new">NUEVO</b>':''}</article>`).join(''):'<div class="rcv34-empty">No tienes mensajes pendientes.</div>';
      $('r34Panel').innerHTML=`
        ...
        <div class="rcv34-section-head"><div><h2>Bandeja de seguimiento</h2><p>Consulta mensajes, respuestas y el estado actual de cada incidencia.</p></div><button id="r34NotifRefresh" class="rcv34-btn">↻ Actualizar</button></div><div class="rcv34-notif-list">${html}</div>`;
      ...
      document.querySelectorAll('[data-notif-module]').forEach(el=>el.onclick=()=>navigate(el.dataset.notifModule));
