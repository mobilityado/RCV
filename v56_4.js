...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  function compareTotals(rows,module){
    const a=totals(compareRows(rows,S.compareBase,S.compareBaseMonth),module);
    const b=totals(compareRows(rows,S.compareTarget,S.compareTargetMonth),module);
  ...
  function comparePeriodLabel(){
    ...
    return `${bl} ${S.compareBase||'—'} vs ${tl} ${S.compareTarget||'—'}`;
  }
  function periodControls(rows){
    ...
    return `<article class="rcv34-card rcv43-period-card rcv481-period r49-filter-card">
      ...
      <div class="r49-filter-body">
        <div class="rcv43-period-title"><div><b>Análisis del periodo</b><span>${S.compareYears?'Selecciona exactamente qué año y qué mes quieres enfrentar.':'Consulta un año completo o un rango de meses.'}</span></div>
        ...
        <div class="r50-compare-selectors">
          <div class="r50-compare-side base"><span>PERIODO BASE</span><label>Año<select id="r481Base">${compareOpts(S.compareBase)}</select></label><label>Mes<select id="r50BaseMonth">${monthOpts(S.compareBaseMonth,'Acumulado del año')}</select></label></div>
          <div class="r50-versus">VS</div>
          <div class="r50-compare-side target"><span>PERIODO A COMPARAR</span><label>Año<select id="r481Target">${compareOpts(S.compareTarget)}</select></label><label>Mes<select id="r50TargetMonth">${monthOpts(S.compareTargetMonth,'Acumulado del año')}</select></label></div>
        </div>
  ...
  function bindPeriodControls(){
    const y=$('r43Year'),f=$('r43From'),t=$('r43To'),base=$('r481Base'),target=$('r481Target'),bm=$('r50BaseMonth'),tm=$('r50TargetMonth');
    const rerender=()=>{
      S.periodYear=y?y.value:S.periodYear;S.periodFrom=f?f.value:S.periodFrom;S.periodTo=t?t.value:S.periodTo;
      if(base)S.compareBase=base.value;if(target)S.compareTarget=target.value;if(bm)S.compareBaseMonth=bm.value;if(tm)S.compareTargetMonth=tm.value;
      renderCurrentSource()
    };
    if(y)y.onchange=rerender;if(f)f.onchange=rerender;if(t)t.onchange=rerender;if(base)base.onchange=rerender;if(target)target.onchange=rerender;if(bm)bm.onchange=rerender;if(tm)tm.onchange=rerender;
  ...
  function compareExecutiveBlock(regionRows,module){
    ...
    const aRows=compareRows(regionRows,S.compareBase,S.compareBaseMonth),bRows=compareRows(regionRows,S.compareTarget,S.compareTargetMonth),c=compareTotals(regionRows,module),mode=breakdownMode();
    ...
    const firstLabel=mode==='brand'?'Marca afectada':'Cuenta contable', countLabel=mode==='brand'?'marcas':'cuentas';
  ...
  function accountCompareMini(regionRows,module,account){
    if(!S.compareYears)return'';
    const ar=compareRows(regionRows,S.compareBase,S.compareBaseMonth).filter(r=>accountName(r)===account),br=compareRows(regionRows,S.compareTarget,S.compareTargetMonth).filter(r=>accountName(r)===account);
  ...
  function openTopModal(data,module,type,meta={},source='cloud'){
    const baseRows=filterRegion(data),rows=S.compareYears?compareRows(baseRows,S.compareTarget,S.compareTargetMonth):filterPeriod(baseRows);
  ...
  function openAccount(data,module,account,meta,source='cloud'){
    const regionRows=filterRegion(data),viewRows=S.compareYears?compareRows(regionRows,S.compareTarget,S.compareTargetMonth):filterPeriod(regionRows),rows=viewRows.filter(r=>accountName(r)===account),t=totals(rows,module);
    const affected=groupRows(rows,managerName),concepts=groupRows(rows,accountDetailName),centers=new Set(rows.map(centerName).filter(Boolean)),areas=new Set(rows.map(areaName).filter(Boolean));
    const key=`CUENTA CONTABLE: ${account}`;
    ...
    $('r34DetailTitle').textContent=account;$('r34DetailSub').textContent=`${MODULES[module].label} · ${adminRegionLabel()} · Cuenta contable`;

    const renderSummary=()=>{
      const affectedCards=[...affected].sort((a,b)=>Math.abs(totals(b[1],module).real)-Math.abs(totals(a[1],module).real)).map(([name,rs])=>{
        const z=totals(rs,module),areaList=[...new Set(rs.map(areaName).filter(Boolean))].slice(0,4);
        return `<button class="r50-impact-card r501-impact-click ${z.st==='red'?'is-red':'is-green'}" data-r501-impact="${esc(name)}"><div class="top"><div><strong>${esc(name)}</strong><small>${areaList.map(esc).join(' · ')||'Sin área identificada'}</small></div>${sema(z.st)}</div><div class="amounts"><span>Real<b>${money(z.real)}</b></span><span>Presupuesto<b>${money(z.budget)}</b></span><span>Diferencia<b>${money(z.diff)}</b></span></div><em>VER DETALLE →</em></button>`
      }).join('');
      const conversation=source==='cloud'&&meta?.snapshotId?threadBlock(module,meta.snapshotId,adminRegionLabel(),key,[...affected.keys()]):'';
      const plan=t.st==='red'&&meta?.snapshotId?`<div class="rcv34-plan"><strong>Plan de acción para esta cuenta</strong><p>Registra la acción correctiva que deberá atender la gerencia responsable.</p><textarea id="r34PlanText" placeholder="Describe la causa, acción, responsable y seguimiento..."></textarea><div class="rcv34-plan-actions"><span id="r34PlanMsg" class="rcv34-msg"></span><button id="r34SavePlan" class="rcv34-btn primary">Guardar plan de acción</button></div></div>`:'';
      $('r34DetailBody').innerHTML=`<div class="rcv48-breadcrumb"><b>${esc(adminRegionLabel())}</b><i>›</i><b>Cuenta contable</b><i>›</i><span>${esc(account)}</span></div>
        <div class="r501-account-head"><div class="rcv34-kpis"><article class="rcv34-kpi"><small>REAL</small><strong>${money(t.real)}</strong></article><article class="rcv34-kpi"><small>PRESUPUESTO</small><strong>${money(t.budget)}</strong></article><article class="rcv34-kpi"><small>VARIACIÓN</small><strong>${money(t.diff)}</strong></article><article class="rcv34-kpi"><small>GERENCIAS AFECTADAS</small><strong>${affected.size}</strong><span>${areas.size} áreas · ${centers.size} centros</span></article></div>
        <button id="r501MainSemaphore" class="r501-main-semaphore ${t.st}"><span>ESTADO GENERAL DE LA CUENTA</span>${sema(t.st)}<small>Toca aquí para abrir el desglose detallado</small></button></div>
        ${accountCompareMini(regionRows,module,account)}
        <article class="rcv34-card"><div class="rcv42-top-head"><div><h3>¿A qué afecta esta cuenta?</h3><p>Selecciona una gerencia o toca el semáforo general para consultar el desglose completo.</p></div><span>${affected.size} gerencias</span></div><div class="r50-impact-grid">${affectedCards||'<div class="rcv34-empty">Sin impactos identificados.</div>'}</div></article>
        ${plan}${conversation}`;
      $('r501MainSemaphore').onclick=()=>renderDetail();
      document.querySelectorAll('[data-r501-impact]').forEach(b=>b.onclick=()=>renderDetail(b.dataset.r501Impact));
      if($('r34SavePlan'))$('r34SavePlan').onclick=()=>savePlan(module,meta.snapshotId,key);
    ...
    const renderDetail=(manager='')=>{
      const detailRows=manager?rows.filter(r=>managerName(r)===manager):rows,detailConcepts=groupRows(detailRows,accountDetailName),dt=totals(detailRows,module);
      const makeRows=list=>list.map(r=>`<tr data-detail-state="${rowState(r)}"><td>${esc(r.account)}</td><td>${esc(managerName(r))}</td><td>${esc(areaName(r))}</td><td>${esc(centerName(r))}</td><td>${esc(r.period)}</td><td>${money(r.real)}</td><td>${money(r.budget)}</td><td>${money(module==='productividad'?r.real-r.budget:Math.abs(r.real)-Math.abs(r.budget))}</td><td>${sema(rowState(r))}</td></tr>`).join('');
      const conceptCards=[...detailConcepts].sort((a,b)=>Math.abs(totals(b[1],module).real)-Math.abs(totals(a[1],module).real)).map(([name,rs])=>{const z=totals(rs,module);return `<article class="rcv34-subhier ${z.st==='red'?'is-red':'is-green'}"><div><strong>${esc(name)}</strong>${sema(z.st)}</div><span>Real <b>${money(z.real)}</b></span><span>Presupuesto <b>${money(z.budget)}</b></span></article>`}).join('');
      $('r34DetailBody').innerHTML=`<div class="r501-detail-toolbar"><button id="r501BackSemaphore" class="rcv34-btn">← Volver al semáforo</button><div><b>${esc(account)}</b><span>${manager?'Gerencia: '+esc(manager):'Desglose completo de la cuenta'}</span></div></div>
        <div class="rcv34-kpis"><article class="rcv34-kpi"><small>REAL</small><strong>${money(dt.real)}</strong></article><article class="rcv34-kpi"><small>PRESUPUESTO</small><strong>${money(dt.budget)}</strong></article><article class="rcv34-kpi"><small>VARIACIÓN</small><strong>${money(dt.diff)}</strong></article><article class="rcv34-kpi"><small>MOVIMIENTOS</small><strong>${detailRows.length}</strong><span>${manager?esc(manager):'Toda la cuenta'}</span></article></div>
        <article class="rcv34-card"><h3>Conceptos que integran la cuenta</h3><p>${manager?'Conceptos asociados a '+esc(manager):'Desglose de conceptos dentro de la cuenta seleccionada.'}</p><div class="rcv34-subhier-grid">${conceptCards}</div></article>
        <article class="rcv34-card"><div class="rcv41-detail-head"><div><h3>Movimientos detallados</h3><p id="r41DetailCount">${detailRows.length.toLocaleString()} movimientos.</p></div><div class="rcv42-detail-tools"><div class="rcv41-detail-filters"><button class="rcv41-state-btn active" data-r41-state="all">Todos</button><button class="rcv41-state-btn red" data-r41-state="red">Rojos</button><button class="rcv41-state-btn green" data-r41-state="green">Verdes</button></div><input id="r42DetailSearch" class="rcv42-search" placeholder="Buscar concepto, gerencia, área, centro…"></div></div>
          <div class="rcv34-table-wrap"><table class="rcv34-table"><thead><tr><th>Cuenta / concepto</th><th>Gerencia</th><th>Área</th><th>Centro / afectación</th><th>Mes</th><th>Real</th><th>Presupuesto</th><th>Diferencia</th><th>Estado</th></tr></thead><tbody id="r41DetailRows">${makeRows(detailRows)}</tbody></table></div><div id="r41DetailEmpty" class="rcv41-detail-empty rcv34-hidden">No hay movimientos con el filtro seleccionado.</div>
        </article>`;
      $('r501BackSemaphore').onclick=renderSummary;
      let detailState='all';const apply=state=>{detailState=state;const q=upper($('r42DetailSearch')?.value||'');let shown=0;document.querySelectorAll('#r41DetailRows tr').forEach(tr=>{const ok=(state==='all'||tr.dataset.detailState===state)&&(!q||upper(tr.textContent).includes(q));tr.classList.toggle('rcv34-hidden',!ok);if(ok)shown++});document.querySelectorAll('[data-r41-state]').forEach(b=>b.classList.toggle('active',b.dataset.r41State===state));if($('r41DetailCount'))$('r41DetailCount').textContent=`${shown.toLocaleString()} movimientos mostrados de ${detailRows.length.toLocaleString()}.`;if($('r41DetailEmpty'))$('r41DetailEmpty').classList.toggle('rcv34-hidden',shown>0)};
      document.querySelectorAll('[data-r41-state]').forEach(b=>b.onclick=()=>apply(b.dataset.r41State));if($('r42DetailSearch'))$('r42DetailSearch').oninput=()=>apply(detailState);
    };
  ...
  function breakdownModeSelector(){const m=breakdownMode();return `<div class="r561-breakdown-mode" style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin:0 0 14px"><span style="font-size:12px;font-weight:800;color:#64748b;margin-right:4px">ORDEN DEL DESGLOSE</span><button class="rcv34-btn ${m==='account'?'primary':''}" data-r561-mode="account">Cuenta contable → Marca afectada</button><button class="rcv34-btn ${m==='brand'?'primary':''}" data-r561-mode="brand">Marca afectada → Cuenta contable</button></div>`}
  function openBrand(data,module,brand,meta,source='cloud'){
    const regionRows=filterRegion(data),viewRows=S.compareYears?compareRows(regionRows,S.compareTarget,S.compareTargetMonth):filterPeriod(regionRows),rows=viewRows.filter(r=>managerName(r)===brand),t=totals(rows,module),accounts=groupRows(rows,accountName);
    const brandLabel=brandDisplayName(brand,rows);$('r34DetailTitle').textContent=brandLabel;$('r34DetailSub').textContent=`${MODULES[module].label} · ${adminRegionLabel()} · Marca afectada`;
    const cards=[...accounts].sort((a,b)=>Math.abs(totals(b[1],module).real)-Math.abs(totals(a[1],module).real)).map(([name,rs])=>{const z=totals(rs,module);return `<button class="r50-impact-card r501-impact-click ${z.st==='red'?'is-red':'is-green'}" data-r561-brand-account="${esc(name)}"><div class="top"><div><strong>${esc(name)}</strong><small>${rs.length.toLocaleString()} movimientos</small></div>${sema(z.st)}</div><div class="amounts"><span>Real<b>${money(z.real)}</b></span><span>Presupuesto<b>${money(z.budget)}</b></span><span>Diferencia (+/-)<b>${money(z.diff)}</b></span></div><em>VER DESGLOSE →</em></button>`}).join('');
    $('r34DetailBody').innerHTML=`<div class="rcv48-breadcrumb"><b>${esc(adminRegionLabel())}</b><i>›</i><b>Marca afectada</b><i>›</i><span>${esc(brandLabel)}</span></div><div class="r501-account-head"><div class="rcv34-kpis"><article class="rcv34-kpi"><small>REAL</small><strong>${money(t.real)}</strong></article><article class="rcv34-kpi"><small>PRESUPUESTO</small><strong>${money(t.budget)}</strong></article><article class="rcv34-kpi"><small>VARIACIÓN (+/-)</small><strong>${money(t.diff)}</strong></article><article class="rcv34-kpi"><small>CUENTAS CONTABLES</small><strong>${accounts.size}</strong><span>${rows.length.toLocaleString()} movimientos</span></article></div></div><article class="rcv34-card"><div class="rcv42-top-head"><div><h3>Cuentas que afectan a esta marca</h3><p>Selecciona una cuenta contable para abrir el mismo desglose detallado de siempre.</p></div><span>${accounts.size} cuentas</span></div><div class="r50-impact-grid">${cards||'<div class="rcv34-empty">Sin cuentas identificadas.</div>'}</div></article>`;
  ...
  function renderData(data,module,meta={},source='cloud'){
    ...
    const rows=S.compareYears?compareRows(regionRows,S.compareTarget,S.compareTargetMonth):filterPeriod(regionRows);
    ...
    const head=`<div class="rcv34-breadcrumb"><span>Región</span><b>${esc(adminRegionLabel())}</b><i>›</i><strong>${mode==='brand'?'Marcas afectadas':'Cuentas contables'}</strong></div>
      <div class="rcv42-kpis"><article><small>REAL</small><strong>${money(t.real)}</strong><span>${esc(activeLabel)}</span></article><article><small>PRESUPUESTO</small><strong>${money(t.budget)}</strong><span>Periodo seleccionado</span></article><article><small>VARIACIÓN</small><strong>${money(t.diff)}</strong></article><article><small>${module==='productividad'?'CUMPLIMIENTO':'USO DE PRESUPUESTO'}</small><strong>${compliance}</strong><span>${bad} ${mode==='brand'?'marcas':'cuentas'} rojas · ${good} verdes</span></article><article class="${bad?'alert':''}"><small>${mode==='brand'?'MARCAS ROJAS':'CUENTAS ROJAS'}</small><strong>${bad}</strong><span>de ${activeGroups.size} ${mode==='brand'?'marcas':'cuentas'}</span></article></div>`;
    ...
    host.insertAdjacentHTML('beforeend',r503ContextChips(module,source)+periodControls(regionRows)+compareExecutiveBlock(regionRows,module)+executiveReportBar(rows,module,meta,source)+head+
      ...
      chart+
      `<article class="rcv34-card"><div class="rcv34-card-head"><div><span class="r503-section-kicker">ANÁLISIS CONTABLE</span><h3>${mode==='brand'?'Marcas afectadas de la región':'Cuentas contables de la región'}</h3><p>${mode==='brand'?'Selecciona una marca para ver las cuentas contables que la afectan y después abrir el desglose.':'Selecciona una cuenta para ver las marcas afectadas y después abrir el desglose.'}</p></div>${noteBtn}</div>
        ${breakdownModeSelector()}
        <div class="r473-unit-filter"><div><span>${mode==='brand'?'FILTRAR MARCAS':'FILTRAR CUENTAS'}</span><strong id="r473UnitCount">${activeGroups.size} de ${activeGroups.size} visibles</strong></div><div class="r473-filter-buttons"><button class="active" data-r473-filter="all">Todas <b>${activeGroups.size}</b></button><button class="red" data-r473-filter="red">Rojas <b>${bad}</b></button><button class="green" data-r473-filter="green">Verdes <b>${good}</b></button></div></div>
        <div class="rcv34-breakdown r50-account-grid" id="r473UnitGrid">${breakdownItems}</div><div id="r473UnitEmpty" class="r473-empty rcv34-hidden">No hay ${mode==='brand'?'marcas':'cuentas'} con el estado seleccionado.</div>
      </article>`);
