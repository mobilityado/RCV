(() => {
  ...
  async function post(params){
    // v55.0: POST real por formulario oculto + verificación prolongada del snapshot. En algunos navegadores,
...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  async function publishLocal(module){
    ...
    try{
      ...
      msg.textContent='Comprimiendo '+S.local.rows.length.toLocaleString()+' registros para publicar…';
      const packed=await gzipBase64(rawPayload);
      msg.textContent='Enviando a la nube…';
      const sendParams={accion:'v34_upload',token:S.session.token,module};
      if(packed){sendParams.payload_gzip=packed;sendParams.payload_encoding='gzip-base64'}else sendParams.payload=rawPayload;
      // El formulario puede disparar onload antes de que podamos leer la respuesta cross-origin.
      // La confirmación real siempre se hace por snapshot nuevo en v34_history.
      post(sendParams).catch(()=>{});
      let confirmed=null;
      ...
      if(!confirmed)throw new Error('La nube NO confirmó una publicación nueva. El portal evitó mostrar “publicado correctamente”. Revisa que el Apps Script v55.1 esté implementado como nueva versión y que config.js apunte a esa implementación.');
