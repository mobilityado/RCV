(() => {
  ...
  async function post(params){
    // v55.4: transporte POST restaurado desde la versión que publicaba correctamente: POST real por formulario oculto + verificación prolongada del snapshot. En algunos navegadores,
...
<nav class="r49-bottom-nav" id="r49BottomNav">
  ...
  async function publishLocal(module){
    if(!S.local)return;
    const b=$('r34Publish'),msg=$('r34UploadMsg');
    b.disabled=true;
    try{
      ...
      if(invalid.length)throw new Error('No se publicará: todavía hay regiones inválidas en el archivo local: '+invalid.join(', '));
      msg.textContent='Publicando y verificando en la nube…';
      msg.className='rcv34-msg';
      const before=await jsonp({accion:'v34_history',token:S.session.token,module});
      const beforeId=before?.items?.[0]?.snapshotId||'';
      // v55.4: mantenemos la corrección de regiones, pero restauramos exactamente el
      // mecanismo de publicación que sí funcionaba antes: formulario POST con payload JSON normal.
      // No comprimimos ni cambiamos el transporte de la carga.
      msg.textContent='Enviando a la nube…';
      const rawPayload=JSON.stringify(S.local);
      const packed=await gzipBase64(rawPayload);
      if(packed){
        msg.textContent='Enviando publicación comprimida a la nube…';
        await post({accion:'v34_upload',token:S.session.token,module,payload_gzip:packed,payload_encoding:'gzip-base64'});
      }else{
        msg.textContent='Enviando publicación a la nube…';
        await post({accion:'v34_upload',token:S.session.token,module,payload:rawPayload});
      }
      let confirmed=null;
      // Archivos grandes pueden tardar bastante en serializarse y escribirse en Sheets.
      // Verificamos hasta ~2 minutos y toleramos fallos transitorios de lectura del historial.
      for(let i=0;i<60;i++){
        msg.textContent='Publicando y verificando en la nube… '+(i+1)+'/60';
        await new Promise(r=>setTimeout(r,2000));
        try{
          const h=await jsonpLong({accion:'v34_history',token:S.session.token,module},30000);
          const latest=h?.items?.[0];
          if(latest?.snapshotId && latest.snapshotId!==beforeId){confirmed=latest;break}
        }catch(_){ }
      }
      if(!confirmed)throw new Error('La nube NO confirmó una publicación nueva. El portal evitó mostrar “publicado correctamente”. Revisa que el Apps Script esté implementado como nueva versión y que config.js apunte a esa implementación.');
